...
extern "C" inline gcoclock_t
gcoclock() // TODO: not thread safe; separate begin/end so that end doesn't have
           // to check for query frequency
{
  gcoclock_t result = 0;
...
// Singly-linked list helper functions; works on any struct with a 'next'
// member.
template <typename T> void slist_clear(T *&head) {
  while (head) {
    T *temp = head;
    head = head->next;
    delete temp;
...
template <typename T> void slist_prepend(T *&head, T *val) {
  val->next = head;
  head = val;
}
...
GCoptimization::GCoptimization(SiteID nSites, LabelID nLabels)
    : m_num_labels(nLabels), m_num_sites(nSites), m_datacostIndividual(0),
      m_smoothcostIndividual(0), m_labelcostsAll(0), m_labelcostsByLabel(0),
      m_labelcostCount(0), m_smoothcostFn(0), m_datacostFn(0),
      m_numNeighborsTotal(0),
      m_queryActiveSitesExpansion(
          &GCoptimization::queryActiveSitesExpansion<DataCostFnFromArray>),
      m_setupDataCostsSwap(0), m_setupDataCostsExpansion(0),
      m_setupSmoothCostsSwap(0), m_setupSmoothCostsExpansion(0),
      m_applyNewLabeling(0), m_updateLabelingDataCosts(0),
      m_giveSmoothEnergyInternal(0),
      m_solveSpecialCases(
          &GCoptimization::solveSpecialCases<DataCostFnFromArray>),
      m_datacostFnDelete(0), m_smoothcostFnDelete(0),
      m_random_label_order(false), m_verbosity(0), m_labelingInfoDirty(true),
      m_lookupSiteVar(new SiteID[nSites]), m_labeling(new LabelID[nSites]),
      m_labelTable(new LabelID[nLabels]),
      m_labelingDataCosts(new EnergyTermType[nSites]),
      m_labelCounts(new SiteID[nLabels]),
      m_activeLabelCounts(new SiteID[m_num_labels]), m_stepsThisCycle(0),
...
template <>
GCoptimization::SiteID
GCoptimization::queryActiveSitesExpansion<GCoptimization::DataCostFnSparse>(
    LabelID alpha_label, SiteID *activeSites) {
  return ((DataCostFnSparse *)m_datacostFn)
      ->queryActiveSitesExpansion(alpha_label, m_labeling, activeSites);
}

//-------------------------------------------------------------------

template <>
void GCoptimization::setupDataCostsExpansion<GCoptimization::DataCostFnSparse>(
    SiteID size, LabelID alpha_label, EnergyT *e, SiteID *activeSites) {
...
template <>
void GCoptimization::applyNewLabeling<GCoptimization::DataCostFnSparse>(
    EnergyT *e, SiteID *activeSites, SiteID size, LabelID alpha_label) {
  DataCostFnSparse *dc = (DataCostFnSparse *)m_datacostFn;
  DataCostFnSparse::iterator dciter = dc->begin(alpha_label);
  for (SiteID i = 0; i < size; i++) {
    if (e->get_var(i) == 0) {
  ...
  updateLabelingInfo(
      false, true,
      false); // labels have changed, so update necessary labeling info
}
...
void GCoptimization::specializeDataCostFunctor(const UserFunctor f) {
  ...
  m_queryActiveSitesExpansion =
      &GCoptimization::queryActiveSitesExpansion<UserFunctor>;
  m_setupDataCostsExpansion =
      &GCoptimization::setupDataCostsExpansion<UserFunctor>;
  ...
  m_updateLabelingDataCosts =
      &GCoptimization::updateLabelingDataCosts<UserFunctor>;
...
void GCoptimization::specializeSmoothCostFunctor(const UserFunctor f) {
  ...
  m_giveSmoothEnergyInternal =
      &GCoptimization::giveSmoothEnergyInternal<UserFunctor>;
  m_setupSmoothCostsExpansion =
      &GCoptimization::setupSmoothCostsExpansion<UserFunctor>;
...
GCoptimization::EnergyType GCoptimization::giveSmoothEnergyInternal() {
  ...
  for (i = 0; i < m_num_sites; i++) {
    ...
    for (n = 0; n < numN; n++) {
      ...
      if (nSite < i)
        eng += weights[n] *
               (sc->compute(i, nSite, m_labeling[i], m_labeling[nSite]));
    }
  }
...
OLGA_INLINE void GCoptimization::addterm1_checked(EnergyT *e, VarID i,
                                                  EnergyTermType e0,
                                                  EnergyTermType e1) {
  if (e0 > GCO_MAX_ENERGYTERM || e1 > GCO_MAX_ENERGYTERM)
    handleError("Data cost term was larger than GCO_MAX_ENERGYTERM; danger of "
                "integer overflow.");
...
OLGA_INLINE void GCoptimization::addterm1_checked(EnergyT *e, VarID i,
                                                  ...
                                                  EnergyTermType w) {
  if (e0 > GCO_MAX_ENERGYTERM || e1 > GCO_MAX_ENERGYTERM)
    handleError("Smooth cost term was larger than GCO_MAX_ENERGYTERM; danger "
                "of integer overflow.");
  if (w > GCO_MAX_ENERGYTERM)
    handleError("Smoothness weight was larger than GCO_MAX_ENERGYTERM; danger "
                "of integer overflow.");
  m_beforeExpansionEnergy += e1 * w;
  e->add_term1(i, e0 * w, e1 * w);
}

OLGA_INLINE void GCoptimization::addterm2_checked(
    EnergyT *e, VarID i, VarID j, EnergyTermType e00, EnergyTermType e01,
    EnergyTermType e10, EnergyTermType e11, EnergyTermType w) {
  if (e00 > GCO_MAX_ENERGYTERM || e11 > GCO_MAX_ENERGYTERM ||
      e01 > GCO_MAX_ENERGYTERM || e10 > GCO_MAX_ENERGYTERM)
    handleError("Smooth cost term was larger than GCO_MAX_ENERGYTERM; danger "
                "of integer overflow.");
  if (w > GCO_MAX_ENERGYTERM)
    handleError("Smoothness weight was larger than GCO_MAX_ENERGYTERM; danger "
                "of integer overflow.");
  // Inside energy/maxflow code the submodularity check is performed as an
  // assertion, but is optimized out. We check it in release builds as well.
  if (e00 + e11 > e01 + e10)
    handleError("Non-submodular expansion term detected; smooth costs must be "
...
GCoptimization::SiteID
GCoptimization::queryActiveSitesExpansion(LabelID alpha_label,
                                          SiteID *activeSites) {
  SiteID size = 0;
...
template <typename DataCostT>
void GCoptimization::setupDataCostsExpansion(SiteID size, LabelID alpha_label,
                                             EnergyT *e, SiteID *activeSites) {
  DataCostT *dc = (DataCostT *)m_datacostFn;
  for (SiteID i = 0; i < size; ++i)
    addterm1_checked(e, i, dc->compute(activeSites[i], alpha_label),
...
template <typename SmoothCostT>
void GCoptimization::setupSmoothCostsExpansion(SiteID size, LabelID alpha_label,
                                               EnergyT *e,
                                               SiteID *activeSites) {
  ...
  for (i = size - 1; i >= 0; i--) {
    ...
    for (n = 0; n < nNum; n++) {
      ...
      if (m_lookupSiteVar[nSite] == -1)
        addterm1_checked(
            e, i, sc->compute(site, nSite, alpha_label, m_labeling[nSite]),
            sc->compute(site, nSite, m_labeling[site], m_labeling[nSite]),
            weights[n]);
      else if (nSite < site) {
        addterm2_checked(
            e, i, m_lookupSiteVar[nSite],
...
template <typename DataCostT>
void GCoptimization::setupDataCostsSwap(SiteID size, LabelID alpha_label,
...
template <typename SmoothCostT>
void GCoptimization::setupSmoothCostsSwap(SiteID size, LabelID alpha_label,
                                          LabelID beta_label, EnergyT *e,
                                          SiteID *activeSites) {
  SiteID i, nSite, site, n, nNum, *nPointer;
  EnergyTermType *weights;
  ...
  for (i = size - 1; i >= 0; i--) {
    ...
    for (n = 0; n < nNum; n++) {
      ...
      if (m_lookupSiteVar[nSite] == -1)
        addterm1_checked(
            e, i, sc->compute(site, nSite, alpha_label, m_labeling[nSite]),
            sc->compute(site, nSite, beta_label, m_labeling[nSite]),
            weights[n]);
      else if (nSite < site) {
        addterm2_checked(e, i, m_lookupSiteVar[nSite],
                         ...
                         sc->compute(site, nSite, beta_label, beta_label),
                         weights[n]);
      }
    }
  }
...
template <typename DataCostT>
void GCoptimization::applyNewLabeling(EnergyT *e, SiteID *activeSites,
                                      SiteID size, LabelID alpha_label) {
  DataCostT *dc = (DataCostT *)m_datacostFn;
  for (SiteID i = 0; i < size; i++) {
    if (e->get_var(i) == 0) {
      SiteID site = activeSites[i];
...
template <typename DataCostT> void GCoptimization::updateLabelingDataCosts() {
  DataCostT *dc = (DataCostT *)m_datacostFn;
  ...
  if (dc && !sc && !lc) {
    ...
    for (SiteID i = 0; i < m_num_sites; ++i) {
      ...
      if (minCostLabel > GCO_MAX_ENERGYTERM)
        handleError("Data cost was larger than GCO_MAX_ENERGYTERM; danger of "
                    "integer overflow.");
...
template <> class GCoptimization::GreedyIter<GCoptimization::DataCostFnSparse> {
public:
  ...
  OLGA_INLINE SiteID feasibleSites() const {
    return (SiteID)(m_siteend - m_site);
  }
...
GCoptimization::EnergyType GCoptimization::solveGreedy() {
  ...
  LabelID *order =
      new LabelID[m_num_labels]; // order[0..activeCount-1] contains the
                                 // activated labels so far
  ...
  try {
    ...
    for (LabelID l = 0; l < m_num_labels; ++l) {
      ...
      for (LabelCostIter *lci = m_labelcostsByLabel[l]; lci; lci = lci->next)
        e[l] += lci->node->cost;
      iter.start(&l);
      e[l] += (EnergyType)(m_num_sites - iter.feasibleSites()) *
              GCO_MAX_ENERGYTERM; // pre-add GCO_MAX_ENERGYTERM for all
                                  // infeasible sites
      for (; !iter.done(); ++iter) {
    ...
    // List of labels in the order that they were expanded upon (order[0] first,
    // order[1] second, ...)
    for (LabelID l = 0; l < m_num_labels; ++l)
      order[l] = l;
    order[alpha] = 0;
    ...
    for (LabelID alpha_count = 1; alpha_count <= m_num_labels; ++alpha_count) {
      ...
      // Energy e[l] for expanding on label 'l' starts at e[alpha] + new
      // labelcosts for introducing l
      ...
      for (iter.start(&order[alpha_count], m_num_labels - alpha_count);
           !iter.done(); ++iter) {
      ...
      // Apply the new labeling, updating m_labelingDataCosts and active
      // labelcosts as necessary
      iter.start(&alpha);
      ...
      for (LabelCostIter *lci = m_labelcostsByLabel[alpha]; lci;
           lci = lci->next)
        lci->node->active = true;
    ...
    if (efinal < estart) {
      ...
      m_labelingInfoDirty = true;
      updateLabelingInfo(
          true, false, false); // update m_labelCounts only; m_labelingDataCosts
                               // and active labelcosts should be up to date
...
void GCoptimization::setDataCostFunctor(DataCostFunctor *f) {
  ...
  m_queryActiveSitesExpansion =
      &GCoptimization::queryActiveSitesExpansion<DataCostFunctor>;
  m_setupDataCostsExpansion =
      &GCoptimization::setupDataCostsExpansion<DataCostFunctor>;
  ...
  m_updateLabelingDataCosts =
      &GCoptimization::updateLabelingDataCosts<DataCostFunctor>;
...
void GCoptimization::setDataCost(LabelID l, SparseDataCost *costs,
                                 SiteID count) {
  if (!m_datacostFn)
    specializeDataCostFunctor(DataCostFnSparse(numSites(), numLabels()));
  else if (m_queryActiveSitesExpansion !=
               (SiteID(GCoptimization::*)(LabelID, SiteID *)) &
           GCoptimization::queryActiveSitesExpansion<DataCostFnSparse>)
    handleError("Cannot apply sparse data costs after dense data costs have "
                "been used.");
  m_labelingInfoDirty = true;
  DataCostFnSparse *dc = (DataCostFnSparse *)m_datacostFn;
...
void GCoptimization::setSmoothCostFunctor(SmoothCostFunctor *f) {
  ...
  m_giveSmoothEnergyInternal =
      &GCoptimization::giveSmoothEnergyInternal<SmoothCostFunctor>;
  m_setupSmoothCostsExpansion =
      &GCoptimization::setupSmoothCostsExpansion<SmoothCostFunctor>;
  m_setupSmoothCostsSwap =
      &GCoptimization::setupSmoothCostsSwap<SmoothCostFunctor>;
}
...
void GCoptimization::setLabelSubsetCost(LabelID *labels, LabelID numLabels,
                                        EnergyTermType cost) {
  if (cost < 0)
    handleError("Label costs must be non-negative.");
  if (cost > GCO_MAX_ENERGYTERM)
    handleError("Label cost was larger than GCO_MAX_ENERGYTERM; danger of "
                "integer overflow.");
  ...
  for (LabelCostIter *lci = m_labelcostsByLabel[labels[0]]; lci;
       lci = lci->next) {
...
GCoptimization::EnergyType GCoptimization::expansion(int max_num_iterations) {
  ...
  try {
    if (max_num_iterations == -1) {
      // Strategic expansion loop focuses on labels that successfuly reduced the
      // energy
      ...
      do {
        ...
        // Make a pass over the unchecked labels in the current queue, i.e.
        // m_labelTable[next..queueSize-1]
        LabelID queueSize = queueSizes.back();
        LabelID start = next;
        m_stepsThisCycleTotal = queueSize - start;
        do {
          if (!alpha_expansion(m_labelTable[next]))
            std::swap(m_labelTable[next],
                      m_labelTable[--queueSize]); // don't put this label in a
                                                  // new queue
          else
        ...
        if (next == start) // No expansion was successful, so try more labels
                           // from the previous queue
        {
          next = queueSizes.back();
          queueSizes.pop_back();
        } else if (queueSize <
                   queueSizes.back() / 2) // Some expansions were successful, so
                                          // focus on them in a new queue
        {
          next = 0;
          queueSizes.push_back(queueSize);
        } else
          next =
              0; // All expansions were successful, so do another complete sweep
  ...
  m_stepsThisCycle = m_stepsThisCycleTotal =
      0; // set so that alpha_expansion() knows it's no inside expansion() if
         // called externally
  return new_energy;
}
...
GCoptimization::setupLabelCostsExpansion(SiteID size, LabelID alpha_label,
                                         EnergyT *e, SiteID *activeSites) {
  ...
  if (m_queryActiveSitesExpansion ==
          (SiteID(GCoptimization::*)(LabelID, SiteID *)) &
      GCoptimization::queryActiveSitesExpansion<DataCostFnSparse>) {
    // For sparse data costs, things are more complicated, because we must
    // ensure that no label cost for a fixed (non-active) non-alpha label is
  ...
  for (LabelCostIter *lci = m_labelcostsByLabel[alpha_label]; lci;
       lci = lci->next)
    lci->node->aux = DISABLE;

  // Since we're explicitly omitting the alpha_label label costs from the binary
  // energy, calculate what it would have been, so that we can potentially
  // reject the expansion afterwards.
  ...
  for (SiteID i = 0; i < size; i++) {
    ...
    for (LabelCostIter *lci = m_labelcostsByLabel[label_i]; lci;
         lci = lci->next) {
...
//-------------------------------------------------------------------
void GCoptimization::updateLabelingInfo(bool updateCounts, bool updateActive,
                                        bool updateCosts) {
  ...
  if (m_labelcostsAll) {
    ...
    if (updateActive) {
      ...
      for (LabelID l = 0; l < m_num_labels; ++l)
        if (m_labelCounts[l])
          for (LabelCostIter *lci = m_labelcostsByLabel[l]; lci;
               lci = lci->next)
            lci->node->active = true;
    }
  }
...
//-------------------------------------------------------------------
// Sets up the binary expansion energy, optimizes it, and updates the current
// labeling.
//
bool GCoptimization::alpha_expansion(LabelID alpha_label) {
  ...
  if (m_stepsThisCycleTotal == 0)
    m_labelingInfoDirty =
        true; // if not inside expansion(), assume data cost function could have
              // changed since last expansion
  updateLabelingInfo();
  ...
  try {
    ...
    EnergyT e(size + m_labelcostCount, // poor guess at number of pairwise terms
                                       // needed :(
              m_numNeighborsTotal +
                  (m_labelcostCount ? size + m_labelcostCount : 0),
    ...
    EnergyType alphaCorrection =
        setupLabelCostsExpansion(size, alpha_label, &e, activeSites);
    checkInterrupt();
...
void GCoptimization::alpha_beta_swap(LabelID alpha_label, LabelID beta_label) {
  assert(alpha_label >= 0 && alpha_label < m_num_labels && beta_label >= 0 &&
         beta_label < m_num_labels);
  if (m_labelcostsAll)
  ...
  if (m_setupDataCostsSwap)
    (this->*m_setupDataCostsSwap)(size, alpha_label, beta_label, &e,
                                  activeSites);
  if (m_setupSmoothCostsSwap)
    (this->*m_setupSmoothCostsSwap)(size, alpha_label, beta_label, &e,
                                    activeSites);
  checkInterrupt();
  e.minimize();
  checkInterrupt();
...
GCoptimizationGridGraph::GCoptimizationGridGraph(SiteID width, SiteID height,
                                                 LabelID num_labels)
...
void GCoptimizationGridGraph::setupNeighbData(SiteID startY, SiteID endY,
...
void GCoptimizationGridGraph::giveNeighborInfo(SiteID site, SiteID *numSites,
                                               SiteID **neighbors,
                                               EnergyTermType **weights) {
...
GCoptimizationGeneralGraph::GCoptimizationGeneralGraph(SiteID num_sites,
                                                       LabelID num_labels)
    : GCoptimization(num_sites, num_labels) {
...
void GCoptimizationGeneralGraph::setNeighbors(SiteID site1, SiteID site2,
                                              EnergyTermType weight) {

  assert(site1 < m_num_sites && site1 >= 0 && site2 < m_num_sites &&
         site2 >= 0);
...
void GCoptimizationGeneralGraph::setAllNeighbors(
    SiteID *numNeighbors, SiteID **neighborsIndexes,
  ...
  m_numNeighborsTotal = 0;
  for (int site = 0; site < m_num_sites; site++)
...
void GCoptimization::printStatus1(const char *extraMsg) {
  ...
  printf("gco>> initial energy: \tE=%lld (E=%lld+%lld+%lld)\n",
         ...
         (long long)giveSmoothEnergy(), (long long)giveLabelEnergy());
  flushnow();
}
...
void GCoptimization::printStatus1(int cycle, bool isSwap, gcoclock_t ticks0) {
  ...
  printf("gco>> after cycle %2d: \tE=%lld (E=%lld+%lld+%lld);", cycle,
         (long long)compute_energy(), (long long)giveDataEnergy(),
         (long long)giveSmoothEnergy(), (long long)giveLabelEnergy());
  if (m_stepsThisCycleTotal > 0)
    printf(isSwap ? " \t%d swaps(s);" : " \t%d expansions(s);",
           m_stepsThisCycleTotal);
  if (m_verbosity == 1) {
    // Don't print time if time is already printed at finer scale, since
    // printing itself takes time (esp in MATLAB) and makes time useless at this
    // level
...
void GCoptimization::printStatus2(int alpha, int beta, int numVars,
                                  gcoclock_t ticks0) {
  if (m_verbosity < 2)
    return;
...
GCoptimization::DataCostFnSparse::DataCostFnSparse(SiteID num_sites,
                                                   LabelID num_labels)
    : m_num_sites(num_sites), m_num_labels(num_labels),
      m_buckets_per_label((m_num_sites + cSitesPerBucket - 1) /
                          cSitesPerBucket),
      m_buckets(0) {}

GCoptimization::DataCostFnSparse::DataCostFnSparse(const DataCostFnSparse &src)
    : m_num_sites(src.m_num_sites), m_num_labels(src.m_num_labels),
      m_buckets_per_label(src.m_buckets_per_label), m_buckets(0) {
...
void GCoptimization::DataCostFnSparse::set(LabelID l,
  ...
  if (!m_buckets) {
    ...
    memset(m_buckets, 0,
           m_num_labels * m_buckets_per_label * sizeof(DataCostBucket));
  }
  ...
  //
  // Scan the list of costs and remember pointers to delimit the 'buckets', i.e.
  // where ranges of SiteIDs lie along the array. Buckets can be empty (begin ==
  // end).
  //
  ...
  for (int i = 0; i < m_buckets_per_label; ++i) {
    ...
    while (next < end && next->site < end_site) {
      if (next->site < 0 || next->site >= m_num_sites)
        throw GCException("Invalid site id given for sparse data cost; must be "
                          "within range.");
      if (next->site <= prev_site)
        throw GCException(
            "Sparse data costs must be sorted in increasing order of SiteID");
      prev_site = next->site;
      ++next;
    }
    b[i].end = next;
...
GCoptimization::EnergyTermType
GCoptimization::DataCostFnSparse::search(DataCostBucket &b, SiteID s) {
  ...
  if (R - L == m_num_sites)
    return b.begin[s]
        .cost; // special case: this particular label is actually dense
  do {
    const SparseDataCost *mid =
        (const SparseDataCost *)((((size_t)L + (size_t)R) >> 1) &
                                 cDataCostPtrMask);
    if (s < mid->site)
  ...
  return GCO_MAX_ENERGYTERM; // the site belongs to this bucket but with no cost
                             // specified
}

OLGA_INLINE GCoptimization::EnergyTermType
GCoptimization::DataCostFnSparse::compute(SiteID s, LabelID l) {
  DataCostBucket &b =
      m_buckets[l * m_buckets_per_label + (s >> cLogSitesPerBucket)];
  if (b.begin == b.end)
  ...
  if ((size_t)b.end - (size_t)b.begin ==
      cSitesPerBucket * sizeof(SparseDataCost))
    return b.begin[s - b.begin->site]
        .cost; // special case: this particular bucket is actually dense!

  return search(b, s);
}

GCoptimization::SiteID
GCoptimization::DataCostFnSparse::queryActiveSitesExpansion(
    LabelID alpha_label, const LabelID *labeling, SiteID *activeSites) {
  const SparseDataCost *next =
      m_buckets[alpha_label * m_buckets_per_label].begin;
  const SparseDataCost *end =
      m_buckets[alpha_label * m_buckets_per_label + m_buckets_per_label - 1]
          .end;
  SiteID count = 0;
  for (; next < end; ++next) {
